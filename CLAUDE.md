...
**Phase 6 (cost-field UI)**, added on top of Phase 5:

- **The write side (`cost` input on `MealEntryDialog`/`RecipeDialog`, and the writable `cost` field
  on both serializers/Zod schemas) already existed** going into this phase — it shipped, undocumented,
  as part of Phase 4's `RecipeDialog` work and Phase 2's `MealEntrySerializer`. What was actually
  missing, and what this phase adds, is the **read side**: nowhere in the UI displayed a saved
  `cost` back to the user. `MealPlanEntry` has no `cost` field (per Phase 5's note above) and stays
  out of scope here — planning a meal doesn't commit to spending anything yet.
- **`/nutrition/log`** (`app/[locale]/nutrition/log/page.tsx`) gained a "Стоимость" table column
  (`entry.cost`, "—" when unset) and a fifth `SummaryStat` tile alongside calories/protein/fat/carbs
  (grid widened `sm:grid-cols-4` → `sm:grid-cols-5`). The daily total is `null` (renders "—" via
  `SummaryStat`'s existing null handling) when **no** entry that day has a cost recorded — summing
  unset entries as an implicit `0` would misrepresent "nobody tracked cost today" as "spent nothing" —
  but once at least one entry has a cost, the rest sum in as `0`, same "show a `—` tile rather than
  hide it" tolerance for partial data as time-in-range/dashboard elements.
- **`/nutrition/recipes`** gained a "Стоимость/порция" (cost per serving) column — `cost / servings`,
  computed client-side same as the dialog's live-estimate arithmetic, rather than a ninth
  `SerializerMethodField` on `RecipeSerializer`: the raw `cost` is already on the wire, and dividing
  by an already-fetched `servings` needs no extra request or backend change. "—" when `cost` is null.
- **`₽` (ruble) introduced as this app's currency unit** — no prior convention existed; picked for
  consistency with the Russian-first UI policy, same role `unit` strings like `"ккал"`/`"г"` already
  play on `SummaryStat`.
- No backend changes this pass — `cost` was already a plain writable `DecimalField` on both
  serializers with no permission gap, so this was purely a frontend display pass; ruff/pytest
  untouched, 254/254 still passing.
- Manually verified end-to-end via browser: created a food item and a 2-serving recipe costed at
  200 ₽, confirmed the recipe list showed exactly 100.00 ₽/serving; logged a meal against the food
  item with cost 45.5, confirmed both the diary row and the new cost `SummaryStat` tile showed
  45.5 ₽; switched to a date with zero entries and confirmed the cost tile fell back to "—" rather
  than "0 ₽". Hit the documented stale-Turbopack-HMR quirk once (the new recipe cost column didn't
  render until `docker compose restart frontend`) — same fix as every prior occurrence.
- Frontend: `eslint`/`tsc --noEmit` clean.

**Duplicate-plan-day** (`feature/nutrition-plan-duplicate-day`), added on top of Phase 6 — not part
of the original phased plan, requested separately once every phase was implemented:

- **`services.duplicate_meal_plan_day(*, user, source_date, target_date)`** copies every one of a
  user's `MealPlanEntry` rows from `source_date` onto `target_date` via `bulk_create` — carries over
  `meal_type`/`food_item`/`recipe`/`quantity_g`/`servings` only, never `resulting_meal_entry`, so a
  duplicated plan always starts unmarked regardless of whether the source day's meal was eaten.
  **Purely additive**: existing plans already on `target_date` are left untouched rather than merged
  or replaced, so re-running a duplicate (or duplicating onto a day that already has plans) just adds
  more rows instead of erroring — same "safe to retry" shape as the bulk-import services. Read side
  reuses the existing `selectors.meal_plan_entry_list_for_user(user=user, date=...)` — no new query
  logic needed, since scoping to one user's one day is already exactly what that selector does.
  `source_date == target_date` is deliberately allowed (doubles that day's plans) rather than
  specially rejected — unusual but valid, not worth extra validation to prevent.
- **`POST /api/meal-plan-entries/duplicate-day/`** (`MealPlanEntryViewSet.duplicate_day`,
  `detail=False` since it isn't scoped to one entry's id) takes `{source_date, target_date}` via a
  small un-modeled `DuplicateMealPlanDaySerializer(serializers.Serializer)` (no model behind it, same
  shape as the formula engine's preview-input serializers) and returns the newly created rows,
  serialized through the same `MealPlanEntrySerializer` every other endpoint on this viewset uses.
  Ownership-scoped through the *service* layer (the selector call inside it), not an object-level
  permission check — there's no single object in the URL to check against, same reasoning
  `dashboard_element`'s per-user actions in `apps.metrics` already use.
- Frontend: `components/nutrition/duplicate-meal-plan-day-button.tsx`'s
  `DuplicateMealPlanDayButton` is a small `Dialog` (not a bare confirm button, since it needs the
  target date as input) — one per day card on `/nutrition/plan`, disabled when that day has nothing
  to copy. Defaults the target-date field to **the same weekday one week later**
  (`addDays(date, 7)`) — the common "repeat this day next week" case — while staying freely editable
  for any other target. On success, invalidates the same `MEAL_PLAN_ENTRIES_QUERY_KEY` every other
  plan mutation already invalidates, so both the source and (if currently visible) target week
  re-fetch correctly.
- Manually verified end-to-end via browser: planned a food item (Гречка QA, 120g) for a Thursday,
  duplicated the day with the default target date, confirmed it landed on the same weekday exactly
  one week later with a fresh "not eaten" state, and confirmed the source day's own plan was
  untouched. Hit the documented plain-shadcn-`Button`-inert-under-this-tooling quirk again for the
  "Дублировать день" trigger — the full synthetic `pointerdown→mousedown→pointerup→mouseup→click`
  sequence fixed it, same as every prior occurrence; not a code issue.
- Backend: 7 new tests in `TestDuplicateDay` (`tests/nutrition/test_meal_plan.py`) — copies every
  entry, empty source day creates nothing, additive onto an already-populated target day, only
  duplicates the requesting user's own entries (cross-user isolation), anonymous rejection, missing
  required field rejection — 260/260 backend tests passing, `ruff check .` clean on every file this
  pass touched. Frontend: `eslint`/`tsc --noEmit` clean.
...
└── frontend/
    ...
    │   │   └── duplicate-meal-plan-day-button.tsx       # dialog: target-date input + POST .../duplicate-day/
...
| Nutrition module Phase 6 — cost-field UI (the write side — `cost` input on `MealEntryDialog`/`RecipeDialog` and the writable `cost` field on both serializers — already existed from Phases 2/4; this phase adds the missing read side: a "Стоимость" table column + a fifth `SummaryStat` daily-total tile on `/nutrition/log` (null, not summed-as-zero, when no entry that day has a cost — same "show a `—` tile" tolerance as time-in-range), and a "Стоимость/порция" client-computed (`cost / servings`) column on `/nutrition/recipes`; `₽` introduced as this app's currency unit — no prior convention existed) | `feature/nutrition-cost-ui` | Implemented, manually verified end-to-end via browser (costed a recipe at 200 ₽/2 servings, confirmed the list showed exactly 100.00 ₽/serving; logged a 45.5 ₽ meal, confirmed both the diary row and the new cost tile showed 45.5 ₽; confirmed a zero-entry date's cost tile fell back to "—" instead of "0 ₽"); no backend changes (cost was already writable with no permission gap), 254/254 backend tests unaffected, `ruff check .` clean; frontend: `eslint`/`tsc --noEmit` clean. Hit the documented stale-Turbopack-HMR quirk once — `docker compose restart frontend` fixed it, same as every prior occurrence. Branched off Phase 5's tip (the latest complete stack). |
| Nutrition — duplicate a planned day (`services.duplicate_meal_plan_day` bulk-copies one day's `MealPlanEntry` rows onto another date, purely additive, never carrying over `resulting_meal_entry`; `POST /api/meal-plan-entries/duplicate-day/` takes `{source_date, target_date}`; frontend `DuplicateMealPlanDayButton` — a per-day-card dialog on `/nutrition/plan`, defaulting the target date to the same weekday one week later, disabled when the day has nothing to copy) — requested directly, not part of the original phased plan | `feature/nutrition-plan-duplicate-day` | Implemented, manually verified end-to-end via browser (duplicated a Thursday's planned meal, confirmed it landed exactly one week later in a fresh "not eaten" state with the source day untouched); backend: 260/260 tests passing incl. 7 new in `TestDuplicateDay`, `ruff check .` clean on every file this pass touched; frontend: `eslint`/`tsc --noEmit` clean. Branched off Phase 6's tip (the latest complete stack). |
| Bugfix: daily-total metrics (Калории/Белки/Жиры/Углеводы (день)) appeared not to update after editing/deleting a meal — two independent root causes. (1) **Frontend cache**: `MealEntryDialog`/`DeleteMealEntryButton`/`MarkMealPlanEntryEatenButton` only invalidated the meal-entries query on success, never the metric-aggregate/metric-entries/dashboard-elements/dashboard-summary queries the materialized daily totals feed — any already-cached view of one of those four metrics (chart, dashboard element, detail page) went stale until an unrelated refetch. New `invalidateDailyNutritionMetricQueries(queryClient)` (`lib/query-keys.ts`) invalidates all four broadly by query-key prefix (the frontend never learns the daily-total metric types' ids to target them specifically, unlike `metric-entry-dialog.tsx`'s own-metric-type invalidation), wired into all three mutation sites. (2) **Future-stamped "today" entries**: `recompute_daily_nutrition_metrics`/`mark_meal_plan_entry_eaten` stamped a day's materialized entry at a fixed noon UTC — a reasonable stand-in for a past day with no natural single timestamp, but for *today* that's frequently still in the future relative to the actual current moment (e.g. any time before 15:00 in Moscow's UTC+3). `current_value_for_metric_type` and every range query filter `recorded_at <= now`, so a future-stamped today's entry was invisible until real time caught up to noon — indistinguishable from "not updating." Both functions now use `timezone.now()` instead of noon when `entry_date`/`plan_entry.date` is today; `existing.save(update_fields=["value", ...])` never touches `recorded_at` on an already-existing row, so this only matters at first-creation-of-the-day, not on every recompute | `fix/nutrition-daily-total-staleness` | Fixed, manually verified end-to-end via browser (logged a meal, confirmed the Углеводы (день) detail page's "Текущее"/chart picked it up immediately with a same-moment timestamp rather than a future noon; edited the quantity and confirmed the diary and the metric detail page — reached via client-side navigation, not a hard reload — both updated live; deleted the entry and confirmed the metric detail page correctly fell back to "no entries" via the same live navigation, with no manual refresh); backend: 262/262 tests passing incl. 2 new regression tests (`test_todays_materialized_entry_is_never_stamped_in_the_future`, `test_marking_a_plan_eaten_today_never_stamps_the_entry_in_the_future`), `ruff check .` clean; frontend: `eslint`/`tsc --noEmit` clean. Branched off `main` (post-merge tip of the nutrition-module branch chain). |
| Search on `/metrics` (`selectors.metric_type_list(*, search=None)` filters by `name__icontains`, same convention as `apps.nutrition.selectors.food_item_list_for_user`/`recipe_list_for_user`; `MetricTypeViewSet.get_queryset` passes through `?search=`; frontend `Input`-driven `search` state feeding `metricTypesQueryKey(search)`/`metricTypes.list(search)`, no debounce — same shape as `/nutrition`'s food-item search) | `feature/metrics-page-search` | Implemented, manually verified end-to-end via browser (typed "талии" and confirmed the list narrowed to just "Обхват талии"; typed a non-matching query and confirmed the "Ничего не найдено" empty state); backend: 265/265 tests passing incl. 3 new (`test_search_filters_by_name`, `test_search_is_case_insensitive_substring_match`, `test_no_search_returns_everything` in `TestMetricTypePermissions`), `ruff check .` clean on every file this pass touched; frontend: `eslint`/`tsc --noEmit` clean. Hit the documented stale-Turbopack-HMR dev-loop quirk once — `docker compose restart frontend` fixed it. |
...
way (Phases 1-2, 4-6 above, on separate branches — Phase 3's Open Food Facts integration is also
implemented on its own sibling branch, `feature/nutrition-off-integration`, not yet merged as of
this branch) per the phased plan in its implementation prompt — every phase of the original plan is
now implemented, pending merge of the outstanding branches into `main`.
