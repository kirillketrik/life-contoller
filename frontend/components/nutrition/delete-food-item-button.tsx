...
import { ApiError, foodItems } from "@/lib/api";
import { FOOD_ITEMS_QUERY_KEY } from "@/lib/query-keys";
import type { FoodItem } from "@/lib/types";

export function DeleteFoodItemButton({ item }: { item: FoodItem }) {
  const t = useTranslations("foodItem");
  const queryClient = useQueryClient();

  const mutation = useMutation({
    mutationFn: () => foodItems.delete(item.id),
    onSuccess: () => {
